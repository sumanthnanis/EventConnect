...
export default function FileUpload({ onUploadComplete }: FileUploadProps) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (files: File[]) => {
      ...
      console.log(
        "FormData size:",
        formData.has("files") ? "Has files" : "No files",
      );
    ...
    (files: FileList) => {
      ...
      if (invalidFiles.length > 0) {
        toast({
          ...
          description: `Only uploading ${validFiles.length} code files. Filtered: ${invalidFiles
            .slice(0, 3)
            .map((f) => f.name)
      ...
      if (validFiles.length === 0) {
        toast({
          ...
          description:
            "Please upload files with these extensions: .js, .jsx, .ts, .tsx, .py, .java, .cpp, .c, .go",
          variant: "destructive",
        });
        return;
      }
      ...
      // Validate file sizes
      const oversizedFiles = processFiles.filter(
  ...
  const traverseDirectory = useCallback(
    async (directoryEntry: FileSystemDirectoryEntry): Promise<File[]> => {
      const files: File[] = [];
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex justify-center space-x-4 text-xs text-slate-500">
              <span>
                <i className="fas fa-file-code mr-1"></i>Max 10MB per file
              </span>
              <span>
                <i className="fas fa-layer-group mr-1"></i>Up to 6 files per
                folder
              </span>
            </div>
          </div>
            ...
            {uploadedFiles.map((uploadedFile) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-8 h-8 bg-${getFileIcon(uploadedFile.file.name).split("-")[1]}-100 rounded flex items-center justify-center`}
                  >
                    <i
                      className={`fas fa-file-code ${getFileIcon(uploadedFile.file.name)} text-sm`}
                    ></i>
                  </div>
                  <div>
